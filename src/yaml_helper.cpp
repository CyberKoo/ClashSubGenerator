...
std::string YAMLHelper::search_key(const YAML::Node &node, const std::vector<std::string> &keys) {
    ...
    throw MissingKeyException(std::format("Unable to find the required key \"{}\"", keys.front()));
}
...
void YAMLHelper::write_yaml(const YAML::Node &node, std::string_view file) {
    ...
    if (fout.is_open()) {
        SPDLOG_INFO("Writing yaml to file {}", file);
        fout << std::format("# Generated by {} at {}\n", get_version(), Utils::get_time("%c"));
        fout << node;
        fout.close();
    } else {
        throw FileWriteException(std::format("Unable to write file {}", file));
    }
}
...
void YAMLHelper::node_renamer(const YAML::Node &node, const std::map<std::string, std::string> &replace_pair) {
    for (auto item: node) {
        ...
        if (replace_pair.count(key_name)) {
            ...
            SPDLOG_TRACE("Replace key {} to {}", key_name, replace_pair.at(key_name));
            continue;
        }
    }
}
